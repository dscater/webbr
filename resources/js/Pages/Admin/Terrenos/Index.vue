...
const { setTerreno, limpiarTerreno } = useTerrenos();
    ...
    },
    {
        ...
        render: function (data, type, row) {
            ...
            if (
                ...
                props_page.auth?.user.permisos.includes("terrenos.edit")
            ) {
            ...
            if (
                props_page.auth?.user.permisos == "*" ||
                props_page.auth?.user.permisos.includes("terrenos.destroy")
            ) {
...
const accionesRow = () => {
    // editar
    $("#table-terreno").on("click", "button.editar", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("terrenos.show", id)).then((response) => {
    ...
    // eliminar
    $("#table-terreno").on("click", "button.eliminar", function (e) {
        e.preventDefault();
...
onMounted(async () => {
    datatable = initDataTable("#table-terreno", columns, route("terrenos.api"));
...
<template>
    ...
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Terreno</h1>
    <!-- END page-header -->
